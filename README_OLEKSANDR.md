# 👋 Hi, I'm Oleksandr Pylypenko - Fullstack Developer

Fullstack Developer · CS Student · ~4–5 years of commercial experience

---

## Summary

I build web apps - both the parts people see and the parts that make them work. Spent the last few years doing outsource and outstaff work, contributed to large Ukrainian marketplaces, and recently started exploring embedded systems. Currently finishing a CS degree and still writing code every day.

---

## 🛠 Technical Skills

**Frontend**
React.js, Next.js, TypeScript, JavaScript (ES6+), Redux (Thunk/Saga), HTML5, CSS3/SASS

**Backend & CMS**
Node.js, Express.js, PHP, Strapi, WordPress

**Databases**
MongoDB, SQL

**Embedded & Tools**
Python, MicroPython, Raspberry Pi, Git, Docker (Basic), Webpack, REST API

---

## 💼 Experience

**Fullstack Development - Outsource & Outstaff**
Built and maintained responsive UIs and RESTful API integrations across multiple client projects in various domains.

**Ukrainian E-commerce Marketplaces**
Worked on high-load frontend interfaces for large-scale Ukrainian marketplaces. Learned what scale actually looks like.

**Community Leadership - 3 years**
Led a volunteer team organizing inclusive sports and educational programs for youth and people with disabilities.

---

## 🎓 Education & Certifications

**B.S. Computer Science** - Kamianets-Podilskyi National University *(In Progress)*

- JavaScript 2024 - Fwdays
- SQL Module - Mate Academy
- MERN Stack - Udemy

---

## 🌍 Languages

- Ukrainian - Native
- English - B1/B2

---

## 🌿 Interests

- 🥊 Muay Thai and functional training
- 🎸 Guitar and music theory
- 🤝 Community development and inclusive fitness initiatives
